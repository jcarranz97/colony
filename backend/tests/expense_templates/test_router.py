...
class TestGetExpenseTemplates:
    def test_list_requires_auth(self, client):
        response = client.get("/api/v1/expense-templates/")
    ...
    def test_list_returns_empty_for_new_user(
        ...
        response = client.get("/api/v1/expense-templates/", headers=headers)
        assert response.status_code == 200
        assert response.json() == []

    def test_list_returns_user_templates(self, client, test_user, test_template):
        headers = get_auth_headers(client, test_user)
        response = client.get("/api/v1/expense-templates/", headers=headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_list_does_not_include_other_users_templates(
        self,
        client,
        test_user,
        other_user,
        ...
        other_template = ExpenseTemplate(
...
class TestCreateExpenseTemplate:
    def test_create_requires_auth(self, client, test_payment_method):
        payload = {**TEMPLATE_PAYLOAD, "payment_method_id": str(test_payment_method.id)}
        response = client.post("/api/v1/expense-templates/", json=payload)
        assert response.status_code == 401
    ...
    def test_create_success(self, client, test_user, test_payment_method):
        ...
        response = client.post(
            "/api/v1/expense-templates/", json=payload, headers=headers
        )
...
class TestGetExpenseTemplateById:
    def test_get_requires_auth(self, client, test_template):
        response = client.get(f"/api/v1/expense-templates/{test_template.id}")
        assert response.status_code == 401
    ...
    def test_get_returns_404_for_other_users_template(
    ...
    def test_get_returns_404_for_nonexistent_id(self, client, test_user):
        headers = get_auth_headers(client, test_user)
        response = client.get(
            f"/api/v1/expense-templates/{uuid.uuid4()}", headers=headers
        )
        assert response.status_code == 404
...
class TestUpdateExpenseTemplate:
    ...
    def test_partial_update_success(self, client, test_user, test_template):
        headers = get_auth_headers(client, test_user)
        response = client.put(
            f"/api/v1/expense-templates/{test_template.id}",
    ...
    def test_update_returns_404_for_other_users_template(
        self, client, other_user, test_template
    ):
        headers = get_auth_headers(client, other_user)
        response = client.put(
...
class TestDeleteExpenseTemplate:
    def test_delete_requires_auth(self, client, test_template):
        response = client.delete(f"/api/v1/expense-templates/{test_template.id}")
        assert response.status_code == 401
    ...
    def test_delete_returns_404_for_other_users_template(
        self, client, other_user, test_template
    ):
        headers = get_auth_headers(client, other_user)
        response = client.delete(
...
class TestHealthCheck:
    def test_health_check_no_auth_required(self, client):
        response = client.get("/api/v1/expense-templates/health")
        assert response.status_code == 200
        assert response.json()["domain"] == "expense_templates"
