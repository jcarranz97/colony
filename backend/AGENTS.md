...
Any change to a SQLAlchemy model requires an Alembic migration. See
[`docs/development/migrations.md`](../docs/development/migrations.md)
for the full deployment workflow (including how to introduce migrations
to an environment that previously used `create_all()`).
...
- Always review the autogenerated file — autogenerate is a starting
...
- The Helm chart runs `alembic upgrade head` automatically in an init
  container before the backend starts; do not call it from `main.py`.
